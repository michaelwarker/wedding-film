...
// Video + Photo bundled together still save 20% off their combined price.
// Photo Booth, when added alongside video and/or photo, gets its own 33%
// discount — applied only to the booth price itself, not the whole order.
function computeCombinedEstimate(video, photo, booth) {
  const videoPhotoBase = (video ? video.price : 0) + (photo ? photo.price : 0);
  const hasVideoPhotoBundle = !!(video && photo);
  const videoPhotoDiscount = hasVideoPhotoBundle ? Math.round(videoPhotoBase * 0.2) : 0;

  const boothBase = booth ? booth.price : 0;
  const hasBoothBundle = !!(booth && (video || photo));
  const boothDiscount = hasBoothBundle ? Math.round(boothBase * 0.33) : 0;

  const base = videoPhotoBase + boothBase;
  const discount = videoPhotoDiscount + boothDiscount;
  const hasBundle = hasVideoPhotoBundle || hasBoothBundle;

  return {
    base, hasBundle, discount,
    total: base - discount,
    hasVideoPhotoBundle, videoPhotoDiscount,
    hasBoothBundle, boothDiscount,
  };
}

function discountLabelFor(combined) {
  if (combined.hasVideoPhotoBundle && combined.hasBoothBundle) return 'Bundle Discounts (20% + 33%)';
  if (combined.hasBoothBundle) return 'Photo Booth Discount (33%)';
  if (combined.hasVideoPhotoBundle) return 'Bundle Discount (20%)';
...
if (estVideo && estPhoto && estTotal) {
  ...
  function updateEstimate() {
    ...
    if (estDiscountLabel) estDiscountLabel.textContent = discountLabelFor(combined);
...
if (boothEstimatorEl) {
  ...
  const boothEstDiscountLabel = document.getElementById('boothEstDiscountLabel');
  ...
  function updateBoothEstimate() {
    ...
    if (boothEstDiscountLabel) boothEstDiscountLabel.textContent = discountLabelFor(combined);
